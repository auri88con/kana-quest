...
- After each navigation the main region takes focus and a visually-hidden live region announces the screen ("Kanji, Learn"), so the app is usable without a mouse or eyes — unless the screen has already placed focus inside its own content region, which it marks with `data-screen-content` (arriving at a radical from a kanji breakdown focuses that card). Focus left on a toolbar tab still resets to `<main>`
